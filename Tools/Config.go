...
func CheckIfConfigExists() {
	...
	if _, err := os.Stat("config/config.json"); os.IsNotExist(err) {
		_, fileErr := os.OpenFile("config/config.json", os.O_CREATE, 0700)
		if fileErr != nil {
...
func GenerateBaseConfig() {
	...
	err = os.WriteFile("config/config.json", conf, 0700)
	if err != nil {
		logger.Fatal(err)
	}
...
func GenerateAzureConfig(azure AzureConfig) {
	...
	err = os.WriteFile("config/azure.json", conf, 0700)
	if err != nil {
		logger.Fatal(err)
	}
}
...
func GenerateSQLConfig(sqlConfig SQLConfig) {
	...
	err = os.WriteFile("config/sql-config.json", conf, 0700)
	if err != nil {
		logger.Fatal(err)
	}
